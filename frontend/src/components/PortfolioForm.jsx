...
const PortfolioForm = ({
  ...
  // Convert old format to new projectLinks format
  useEffect(() => {
    const saved = localStorage.getItem("portfolioFormData");
    if (saved) {
      const parsed = JSON.parse(saved);
      if (Array.isArray(parsed.projectLinks)) {
        parsed.projectLinks = parsed.projectLinks.map((item) =>
          typeof item === "string"
  ...
  const addProject = () => {
    setFormData((prev) => ({
      ...prev,
      projectLinks: [...prev.projectLinks, { link: "", description: "" }],
    }));
  };
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {formData.projectLinks.map((proj, i) => (
              <div key={i} className="my-2">
                <input
                  value={proj.link || ""}
                  onChange={(e) => handleProjectChange(i, "link", e.target.value)}
                  placeholder={`Project ${i + 1} Link`}
            ...
            <button type="button" className="btn mt-1" onClick={addProject}>
              + Add Project
            </button>
          </div>
