...
// frontend/components/LivePreview.jsx
import React, { Suspense } from "react";
...
const LivePreview = ({ selectedTemplate, formData }) => {
  ...
  // Extract projects with names + descriptions
  const projects =
  ...
  const safeProps = {
    name: formData.name || "Your Name",
    title: formData.bio || "Developer",
    github: formData.githubUsername ? `https://github.com/${formData.githubUsername}` : "https://github.com",
    projects,
    socials: Object.entries(formData.socialLinks || {}).map(([platform, link]) => ({
      platform,
      link,
  ...
  return (
    <div className="mt-10 border rounded-lg p-4 bg-white shadow-md">
      ...
      <div className="p-2 max-h-[70vh] overflow-y-auto border rounded-md bg-gray-50">
        <Suspense fallback={<div>Loading Preview...</div>}>
          {/* ✅ Tailwind will work ONLY if styles are global */}
          <SelectedComponent {...safeProps} />
        </Suspense>
      </div>
    </div>
